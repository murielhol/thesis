    ...
    for t, futures in enumerate(futures_stream):
        ...
        if not have_state:
            ...
            if t_sell is not None:
                ...
                if t_sell == 1 or expected_future[1:t_sell].min() > x0:
                    print('BUY BUY BUY')
                    use_dbplot_axis('x0')
            ...
            lost_case = expected_future.max() < x0
            if t_buy is not None or lost_case:
                if t_buy==1 or not expected_future[1:t_buy].max() > x0 or lost_case:
